...
const FlipPage = React.forwardRef(({ pageNumber, scale, isCover }: { pageNumber: number; scale: number; isCover?: boolean }, ref: any) => {
  return (
    <div className="page bg-white shadow-[inset_0_0_20px_rgba(0,0,0,0.1)] overflow-hidden" ref={ref} data-density={isCover ? "hard" : "soft"}>
      <div className="w-full h-full flex items-center justify-center bg-white relative">
        ...
        {!isCover && pageNumber === 2 && (
          <a href="/shop" className="absolute top-[30%] left-[20%] w-[40%] h-[30%] border-2 border-blue-500/0 hover:border-blue-500/50 hover:bg-blue-500/10 z-20 rounded-xl transition-all flex items-center justify-center group cursor-pointer" title="Click to view product">
             <div className="opacity-0 group-hover:opacity-100 bg-blue-600 text-white text-[10px] font-bold px-3 py-1.5 rounded-full shadow-lg transform -translate-y-4 transition-all">Add to Cart - $149</div>
          </a>
        )}
      </div>
    </div>
        ...
        <header className="h-[60px] flex-shrink-0 bg-slate-950 border-b border-white/[0.07] flex items-center justify-between px-4 lg:px-8 text-white z-20">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:block">
              <h3 className="font-heading font-bold text-[14px] text-white leading-tight">{catalog.title}</h3>
              <p className="text-[11px] text-slate-500 font-medium">Interactive Catalog</p>
            </div>
          </div>

          <div className="flex items-center gap-1">
            <button className="w-9 h-9 rounded-lg hover:bg-white/8 text-slate-400 hover:text-white transition-colors flex items-center justify-center" title="Zoom Out" onClick={() => setScale(s => Math.max(0.5, s - 0.2))}>
              <ZoomOut className="w-4 h-4" />
            </button>
            <span className="text-[11px] font-bold text-slate-500 w-10 text-center tabular-nums">{Math.round(scale * 100)}%</span>
            <button className="w-9 h-9 rounded-lg hover:bg-white/8 text-slate-400 hover:text-white transition-colors flex items-center justify-center" title="Zoom In" onClick={() => setScale(s => Math.min(2.5, s + 0.2))}>
              <ZoomIn className="w-4 h-4" />
            </button>

            <div className="w-px h-5 bg-white/10 mx-2" />

            <button className="w-9 h-9 rounded-lg hover:bg-white/8 text-slate-400 hover:text-white transition-colors hidden md:flex items-center justify-center" title="Search"><Search className="w-4 h-4" /></button>
            <button className="w-9 h-9 rounded-lg hover:bg-white/8 text-slate-400 hover:text-white transition-colors hidden md:flex items-center justify-center" title="Bookmark"><Bookmark className="w-4 h-4" /></button>
            <button className="w-9 h-9 rounded-lg hover:bg-white/8 text-slate-400 hover:text-white transition-colors hidden md:flex items-center justify-center" title="Share"><Share2 className="w-4 h-4" /></button>
            <button className="w-9 h-9 rounded-lg hover:bg-white/8 text-slate-400 hover:text-white transition-colors hidden sm:flex items-center justify-center" title="Print" onClick={() => window.print()}><Printer className="w-4 h-4" /></button>
            <a href={catalog.pdfUrl} target="_blank" rel="noreferrer" className="w-9 h-9 rounded-lg hover:bg-white/8 text-slate-400 hover:text-white transition-colors hidden sm:flex items-center justify-center" title="Open in New Tab"><ExternalLink className="w-4 h-4" /></a>
            ...
            <Document
              ...
              loading={
                <div className="flex flex-col items-center gap-4 text-white">
                  ...
                  <p className="text-[13px] font-medium text-slate-400">Loading catalog…</p>
                </div>
              }
            ...
            >
              {isError ? (
                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 max-w-md text-center text-white">
                  ...
                  <p className="text-slate-400 text-sm mb-6">The PDF file might be corrupted or unavailable at the moment. Please try downloading it instead.</p>
