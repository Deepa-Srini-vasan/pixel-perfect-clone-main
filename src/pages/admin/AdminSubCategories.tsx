...
function collectDescendantIds(categories: Category[], parentId: number) {
...
export default function AdminSubCategories() {
  ...
  const subcategoryRoots = useMemo(() => categoryTree.flatMap((root) => root.children), [categoryTree]);
    ...
    nodes.flatMap((node) => [
      ...
      >
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">Parent: {node.category.parentName ?? "Unknown"}</p>
        </div>
  ...
  const renderParentOptions = (nodes: CategoryTreeItem[], depth = 0): React.ReactNode[] =>
    nodes.flatMap((node) => {
      const disabled = excludedParentIds.has(node.category.id) || node.category.id === editingCategory?.id;
      return [
        <option key={node.category.id} value={node.category.id} disabled={disabled}>
          {" ".repeat(depth * 3)}{node.category.name}
        </option>,
        ...renderParentOptions(node.children, depth + 1),
      ];
    });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(payload.error ?? "Failed to save subcategory");
      }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage subcategories and assign each to a parent category.</p>
        </div>
        ...
        <CardContent className="pt-6">
          {isLoading ? (
            ...
            <div className="space-y-2">
              {renderCategoryRows(subcategoryRoots)}
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingCategory ? "Edit Subcategory" : "Add Subcategory"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                {categoryOptions.map((option) => (
                  <option key={option.id} value={option.id} disabled={editingCategory?.id === option.id || excludedParentIds.has(option.id)}>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={handleSave} disabled={isSaving || !formData.name || formData.parentId == null}>
